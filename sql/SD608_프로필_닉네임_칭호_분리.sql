-- Live migration: split_profile_nickname_title_cards
-- 기존 identity 블록을 nickname/title 두 블록으로 호환 변환한다.
-- 카드 레이아웃 version 5.
...
declare
  ...
  v_allowed constant text[] := array['photo','nickname','title','assets','gold','coins'];
  v_key text;
  v_gold_display text;
...
begin
  ...
  if jsonb_typeof(v_raw_layout->'settings'->'coin_codes') = 'array' then
    for v_coin_code in
      ...
      join (select distinct upper(value) as code from jsonb_array_elements_text(v_raw_layout->'settings'->'coin_codes')) requested
  ...
  v_card_layout := jsonb_build_object(
    ...
    'visible',jsonb_build_object('photo',v_show_photo,'nickname',v_show_nickname,'title',v_show_title,'assets',v_show_assets,'gold',v_show_gold,'coins',v_show_coins),
...
begin
  ...
  if not exists (select 1 from public.sd_public_profiles where user_id=v_user_id) then raise exception '먼저 공개 프로필을 만들어 주세요.'; end if;
  if p_layout is null or jsonb_typeof(p_layout) <> 'object' then raise exception '프로필 카드 배치 정보가 올바르지 않습니다.'; end if;
  ...
  if jsonb_typeof(p_layout->'settings'->'coin_codes')='array' then
    for v_coin_code in
      select c.code from public.sd_coins c
      join (select distinct upper(value) as code from jsonb_array_elements_text(p_layout->'settings'->'coin_codes')) requested on requested.code=upper(c.code)
  ...
  v_result := jsonb_build_object('version',5,'order',to_jsonb(v_order),'visible',v_visible,'settings',jsonb_build_object('gold_display',v_gold_display,'coin_codes',to_jsonb(v_coin_codes)));

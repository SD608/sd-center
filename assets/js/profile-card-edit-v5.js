...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  const saveShowcaseButton = document.getElementById("saveEditorShowcase");
    ...
    avatars.forEach((item) => {
      ...
      button.addEventListener("click", async () => {
        ...
        try {
          ...
          if (!profile?.avatar_url || profile.avatar_url !== item.asset_url) button.disabled = false;
        }
      });
    ...
    titles.forEach((item) => {
      ...
      button.addEventListener("click", async () => {
        ...
        try {
          ...
          if (!item.equipped) button.disabled = false;
        }
      });
      row.append(info, button);
  ...
  function renderShowcasePicker() {
    ...
    ownedItems.forEach((item) => {
      const choice = document.createElement("label");
      choice.className = "profile-editor-showcase-choice";
      const input = document.createElement("input");
      input.type = "checkbox";
      input.value = item.id;
      ...
      choice.append(input, info);
      showcasePicker.append(choice);
    });

    if (saveShowcaseButton) saveShowcaseButton.disabled = isSavingShowcase;
  }
  ...
  async function loadCustomizationData() {
    ...
    if (!itemsResult.error) ownedItems = itemsResult.data?.items || [];
    if (!shopResult.error) {
      shopItems = shopResult.data?.items || [];
      achievementTitles = shopResult.data?.achievement_titles || [];
    }
    ...
    if (itemsResult.error || shopResult.error) {
      setStatus("일부 프로필 설정 목록을 불러오지 못했습니다. 다시 열어 주세요.", "error");
    }
  }
  ...
  saveShowcaseButton?.addEventListener("click", async () => {
    ...
    saveShowcaseButton.disabled = true;
    try {
      ...
      saveShowcaseButton.disabled = false;
    }
  });

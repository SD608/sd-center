...
bitcoin-05|bitcoin|BTC NOT FOUND|비트코인 보유량 정확히 404개 달성|h||BTC
...
window.SD_ACHIEVEMENTS=A;window.SD_ACHIEVEMENT_PROGRESS=window.SD_ACHIEVEMENT_PROGRESS||{};
const N=v=>Number(v||0).toLocaleString("ko-KR"),V=(v,u)=>typeof v==="number"?N(v)+(u||""):v;
const card=a=>{if(a.h)return`<article class="achievement-card hidden-achievement"><div class="achievement-top"><div class="achievement-icon">❔</div><span class="achievement-badge hidden-badge">HIDDEN</span></div><h3 class="achievement-name">???</h3><div class="achievement-condition"><small>달성 조건</small><strong>???</strong></div></article>`;
let p="";if(a.p){const cur=Number(window.SD_ACHIEVEMENT_PROGRESS[a.id]||0),pct=typeof a.t==="number"&&a.t>0?Math.max(0,Math.min(100,cur/a.t*100)):0;p=`<div class="achievement-progress"><div class="achievement-progress-head"><span>진행도</span><strong>${pct.toFixed(pct>=10?0:1)}%</strong></div><div class="achievement-progress-track"><div class="achievement-progress-fill" style="width:${pct}%"></div></div><div class="achievement-progress-detail"><span>${N(cur)}${a.u||""}</span><span>목표 ${V(a.t,a.u)}</span></div></div>`}return`<article class="achievement-card"><div class="achievement-top"><div class="achievement-icon">🏆</div><span class="achievement-badge">${a.p?"진행형":"특수 조건"}</span></div><h3 class="achievement-name">${a.n}</h3><div class="achievement-condition"><small>달성 조건</small><strong>${a.d}</strong></div>${p}</article>`};
const render=()=>{const tabs=document.querySelector(".achievement-tabs"),wrap=document.querySelector(".achievement-tabs-wrap"),main=document.querySelector(".achievements-page");if(!tabs||!wrap||!main)return;const count=document.querySelector(".achievement-count");if(count)count.textContent=`등록 업적 · ${A.length}개`;const intro=document.querySelector(".achievements-head p");if(intro)intro.textContent="지금까지 확정된 업적을 콘텐츠별로 확인할 수 있습니다. 히든 업적은 달성 전까지 이름과 조건이 ???로 표시되고, 누적형·보유형·연승형 업적은 진행률을 %로 표시합니다.";tabs.innerHTML="";main.querySelectorAll("[data-achievement-panel]").forEach(x=>x.remove());C.forEach((c,i)=>{const arr=A.filter(a=>a.c===c.id),b=document.createElement("button");b.type="button";b.className="achievement-tab"+(i?"":" active");b.dataset.achievementTab=c.id;b.textContent=`${c.l} (${arr.length})`;tabs.append(b);const s=document.createElement("section");s.className="achievement-panel"+(i?"":" active");s.dataset.achievementPanel=c.id;s.innerHTML=`<div class="achievement-category-title"><div><span>${c.e}</span><h2>${c.l}</h2></div><span class="achievement-category-meta">${arr.length}개 업적</span></div><div class="achievement-grid">${arr.map(card).join("")}</div>`;main.append(s)});const note=document.createElement("div");note.className="achievement-placeholder";note.textContent="업적 조건 94종이 등록되어 있으며, SD코인의 10,000개 보유 업적은 현재 6개 종목별로 분리되어 실제 표시 업적은 총 99개입니다. 진행률은 게임 기록 연동 전에는 0%로 표시됩니다.";main.append(note);tabs.onclick=e=>{const b=e.target.closest("[data-achievement-tab]");if(!b)return;tabs.querySelectorAll(".achievement-tab").forEach(x=>x.classList.toggle("active",x===b));main.querySelectorAll("[data-achievement-panel]").forEach(x=>x.classList.toggle("active",x.dataset.achievementPanel===b.dataset.achievementTab))}};

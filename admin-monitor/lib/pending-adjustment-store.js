...
const fs=require("fs"); const path=require("path"); const {ApiError,MAX_ADJUSTMENT_AMOUNT}=require("./sd-admin-api");
function normalizeAdjustment(payload={}){const requestId=String(payload.requestId||"").trim(); const userId=String(payload.userId||"").trim(); const direction=String(payload.direction||"").trim().toLowerCase(); const amount=Number(payload.amount); const note=String(payload.note||"").trim(); if(!/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(requestId)) throw new ApiError("REQUEST_ID_REQUIRED"); if(!userId) throw new ApiError("WALLET_TARGET_NOT_FOUND"); if(!['credit','debit'].includes(direction)) throw new ApiError("INVALID_DIRECTION"); if(!Number.isSafeInteger(amount)||amount<1||amount>MAX_ADJUSTMENT_AMOUNT) throw new ApiError("INVALID_AMOUNT"); if(note.length>80) throw new ApiError("NOTE_TOO_LONG"); return {requestId,userId,direction,amount,note};}

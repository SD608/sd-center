...
'''
    ...
    text = text.replace(reset_anchor, '''  safeRattleAt = 0;
  safeDialDrag = null;
  modalSpaceAction = confirmSafeDial;
''', 1)
...
'''
    ...
    text = text.replace(bind_anchor, '''  renderSafePins();
...
def patch_json(name: str, raw: bytes) -> bytes:
    ...
    if name == "package.json":
        data["version"] = "1.1.0"
        data["description"] = "SD 플리마켓 PC 확장팩 v1.1.0 - 정상 본체 복구 및 금고 A/D + 마우스 다이얼 조작"
    elif name == "sd-app.json":
        data["version"] = "1.1.0"
        data["displayVersion"] = "PC Expansion · v1.1.0"
    ...
    with zipfile.ZipFile(src, "r") as zin, zipfile.ZipFile(dst, "w", compression=zipfile.ZIP_DEFLATED, compresslevel=9) as zout:
        ...
        for info in zin.infolist():
            ...
            if name == "public/app.js":
                ...
                text = text.replace("v1.0.4", "v1.1.0")
...
def main() -> int:
    if len(sys.argv) != 3:
        print("usage: build-flea-v110.py SOURCE.zip OUTPUT.zip")
        return 2
    src = Path(sys.argv[1])
    dst = Path(sys.argv[2])
    build(src, dst)

...
test("latest corruption falls back to previous valid generation", async () => {
  ...
  try {
    ...
    await storage.writeSnapshot(a);
    await storage.writeSnapshot(b);
...
test("journal checksum and append order survive reopen", async () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "sd-runtime-journal-"));
  try {
    const storage = new RuntimeStorageService(dir);
    await storage.appendJournal("r","e",{authority_epoch:0,commit_seq:1,event_type:"A",payload:{}});
    await storage.appendJournal("r","e",{authority_epoch:0,commit_seq:2,event_type:"B",payload:{}});
